...
| EST-F013 | 🟡 Média | Feature | reserva-estoque-checkout | `StockReservation` com `expires_at`, liberada por scheduler, pela confirmação de pagamento ou pelo cancelamento — evita overselling. **Decisão revista em [`plano-pdv-marketplace.md`](../../plano-pdv-marketplace.md) §2.2: a reserva nasce no _checkout_, não na entrada no carrinho** — carrinho abandonado é a regra, e reservar ali seguraria estoque por horas. Segue a divisão que o módulo já usa: contador em `stock_balance.reserved_quantity` (estado, protegido pelo `@Version` existente) + `stock_reservation` (ledger do quem/porquê/até-quando), escritos na mesma transação. Migration **V64, já escrita**. Falta endpoint, scheduler de expiração e testes — ver [`proximos-passos.md`](proximos-passos.md). Fatia 7. | 🚧 Em andamento |
| EST-F014 | 🔴 Alta | Feature | estorno-devolucao-venda | Devolução (PDV ou e-commerce) gera `StockMovement` de entrada estornando a baixa original, com rastreabilidade da venda de origem; acompanha `REVERSED` no cashback e estorno do pagamento. **Sobe de prioridade**: precisa vir *antes* da reserva (EST-F013), porque reserva sem cancelamento é estoque travado sem como destravar. Fatia 5 do plano. | Backlog (Sprint 4) |
| EST-F015 | 🟡 Média | Feature | kit-produto-composto | Produto "kit"/combo (ex.: kit narguilé = essência + carvão + descartável) que dá baixa nos componentes conforme receita cadastrada. **Decidido em [`plano-pdv-marketplace.md`](../../plano-pdv-marketplace.md) §2.10: kit é _virtual_ e de _um nível só_** — sem saldo próprio (`min(floor(saldo_componente / qtd_receita))`), componente obrigatoriamente `SIMPLES`, o que elimina ciclo e recursão por construção em vez de por travessia. `product.type` + `product_kit_component` (V70). O `order_item` guarda o SKU do kit, mas as movimentações são nos componentes — item e movimentação deixam de ser 1:1, então `stock_movement.reason` precisa carregar o pedido **e** o SKU do kit. Fatia 6. | Backlog (Sprint 6) |
...
| EST-F021 | 🟡 Média | Feature | saida-contra-saldo-disponivel | `StockBalance` ganha `reservedQuantity` (`>= 0`, `<= quantity`, `CHECK` no schema) com `availableQuantity()`, `reserve()`, `releaseReservation()` e `consumeReservation()`. `apply(SAIDA, qty)` passa a validar contra o **disponível**, não contra o físico — senão o balcão vende a unidade que um pedido online pago já reservou. Ponto de atenção: `StockBalance.apply` é método maduro no caminho crítico de PDV, Compras e balanço; `reserved = 0` preserva o comportamento atual por construção, mas a suíte inteira precisa ser revalidada. Domínio e service **já escritos** (`StockBalance.reserve/consumeReservation/releaseReservation`, `EstoqueService.reserveStock`); falta a revalidação da suíte e a mudança de `apply(SAIDA)` para validar contra o disponível. Acompanha EST-F013. | 🚧 Em andamento |
| EST-F022 | 🟢 Baixa | Feature | custo-derivado-de-kit-no-pricing | `findPricingBySku` para um SKU `KIT` devolve `Pricing.of(soma dos custos dos componentes, null, salePrice do kit)`. Com isso `marginPercent()`, `marginAmount()` e `isBelowCost()` funcionam para kits **sem uma linha de matemática nova** — o preço de venda é próprio (é o ponto do kit ser mais barato que a soma), só o custo é derivado. Acompanha EST-F015. | Pendente |
...
| EST-C013 | 🟡 Importante | Correção | integridade-reserva-vs-ledger | Endpoint de integridade confrontando `stock_balance.reserved_quantity` com a soma das reservas `ACTIVE` em `stock_reservation`, no molde de `GET /estoque/integrity/orphan-skus` (EST-C011). Divergência aqui é **estoque travado invisível**: a venda não acontece e ninguém sabe por quê — sintoma mais difícil de diagnosticar que o overselling que a reserva evita. Entra junto com EST-F013/EST-F021. | Pendente |

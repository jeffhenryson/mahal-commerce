...
public interface StockBalanceJpaRepository extends JpaRepository<StockBalanceEntity, Long> {
    ...
    // colidem. Precedência de custo efetivo idêntica a Product.effectivePricingFor: custo próprio
    // da variação, senão o do pai dela, senão o do produto direto, senão zero (saldo sem custo
    // conhecido contribui zero, não invalida a soma).
    //
    ...
    @Query("""
            SELECT COALESCE(SUM(sb.quantity * COALESCE(v.costPrice, vp.costPrice, p.costPrice, 0)), 0)
